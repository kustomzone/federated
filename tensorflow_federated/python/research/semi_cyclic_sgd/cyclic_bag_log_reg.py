...
flags.DEFINE_integer('task_id', -1, 'Borg provided task ID')
...
flags.DEFINE_integer('num_groups', 2, 'Number of groups.')
flags.DEFINE_integer(
...
class CyclicDataGenerator(object):
  """Generates minibatches from data grouped by day & component.
  ...
  def get(self, day, group):
    ...
    for i in range(start_index, end_index, self.batch_size):
      ...
      if i_mod + self.batch_size <= group_size:
        res = self.data[group][i_mod:i_mod + self.batch_size]
        if not res:
...
class NonIidDataGenerator(CyclicDataGenerator):
  ...
  def __init__(self,
    ...
    # Python 2: type(x/2) == int, Python 3: type(x/2) == float. Jesus. Use //.
    biases = np.interp(
